...
exports.addItem = async (req, res) => {
  try {
    const { name, category, unit, quantity, minimum } = req.body;
    if (!name) return res.status(400).json({ error: "Name is required" });
    const existing = await Inventory.findOne({ name });
    if (existing) return res.status(409).json({ error: "Item already exists" });
    const item = await Inventory.create({ name, category, unit, quantity, minimum });
...
exports.updateItem = async (req, res) => {
  try {
    ...
    const item = await Inventory.findByIdAndUpdate(
      itemId,
      { $set: { name, category, unit, quantity, minimum, updatedAt: new Date() } },
      { new: true }
    );
...
exports.deductMaterials = async (materialsUsed = []) => {
  ...
  for (const mat of materialsUsed) {
    await Inventory.findOneAndUpdate(
      { name: mat.name },
      {
        $inc: { quantity: -Math.abs(mat.qty) }, // deduct qty
...
exports.getLowStock = async (req, res) => {
  try {
    ...
    const items = await Inventory.find({ $expr: { $lt: ["$quantity", "$minimum"] } }).sort({ quantity: 1 });
    res.json({ items });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
