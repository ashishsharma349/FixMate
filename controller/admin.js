...
exports.getDashboardStats = async (req, res) => {
  try {
    ...
    // Low stock items for dashboard alert
...
exports.getReportsData = async (req, res) => {
  try {
    ...
    // Category breakdown from complaint titles (using department of assigned staff as proxy)
    ...
    // Revenue from resolved CommonArea complaints
    const revenueData = await Complain.aggregate([
      { $match: { status: "Resolved", workType: "CommonArea" } },
      { $group: { _id: null, totalExpense: { $sum: { $ifNull: ["$actualCost", 0] } } } }
    ]);

    const totalExpense = revenueData[0]?.totalExpense || 0;
...
exports.createUser = async (req, res) => {
  ...
  try {
    ...
    const user = await User.create({
      authId: auth._id, name, age,
      phone: phoneNum, aadhaar,
...
exports.updateUser = async (req, res) => {
  try {
    ...
    const user = await User.findByIdAndUpdate(
      userId,
      { $set: { name, age, phone, aadhaar, flatNumber } },
      { new: true }
    );
    if (!user) return res.status(404).json({ error: "User not found" });
    if (email) await Auth.findByIdAndUpdate(user.authId, { $set: { email } });
...
// ── ADMIN SETTINGS – update own profile ──────────────────────────────────────
